...
export const index = async (req: Request, res: Response) => {
  const products = await ProductModel.getAll(req.session.userId!)
  res.render('products/index', { products })
}

export const show = async (req: Request, res: Response) => {
  const id = parseInt(req.params.id as string)
  const product = await ProductModel.getById(id, req.session.userId!)
...
export const createAction = async (req: Request, res: Response) => {
  ...
  const product = await ProductModel.create({ ...result.data, userId: req.session.userId! })
...
export const editAction = async (req: Request, res: Response) => {
  ...
  try {
    await ProductModel.update(id, req.session.userId!, result.data)
...
export const deleteAction = async (req: Request, res: Response) => {
  const id = parseInt(req.params.id as string)
  try {
    await ProductModel.remove(id, req.session.userId!)
